 ...
 * sola cadena de fórmulas por fila/día. Los bloques comparten la fecha
 * pero NUNCA dependen unos de otros (salvo TOTAL, que el Bloque 1 toma
 * como dato de entrada ya calculado por el Bloque 2 — flujo en un solo
 * sentido: caja -> deuda, nunca deuda -> caja).
 *
 ...
 *   - cobradoHoy              = TOTAL de hoy (efectivo + nequi, calculado en Bloque 2)
...
// ---------------------------------------------------------------------------
// BLOQUE 3 (datos): venta y costo del día.
// venta   = COSTO CLL (valor de venta de lo entregado hoy)
// costo   = costo de inversión de lo entregado hoy
// ---------------------------------------------------------------------------
export function sumDaySale(dayVisits = [], dayItems = [], products = [], dayPayments = [], cashTotal = 0) {
  let venta = 0;
  let costo = 0;
  let unidades = 0;
  const costoFromVisits = (dayVisits || []).reduce((a, v) => a + toNum(v.costo), 0);
  if ((dayItems || []).length > 0) {
    ...
    venta = ventaFromItems > 0
      ? ventaFromItems
      : (dayVisits || []).reduce((a, v) => a + toNum(v.venta), 0);
    unidades = (dayItems || []).length > 0
      ? (dayItems || []).reduce((a, it) => a + toNum(it.quantity), 0)
      : (dayVisits || []).length;
    ...
    if (venta === 0 && (dayPayments || []).length > 0) venta = cashTotal;
  } else {
    venta = (dayVisits || []).reduce((a, v) => a + toNum(v.venta), 0);
    if (venta === 0 && (dayPayments || []).length > 0) venta = cashTotal;
...
// ---------------------------------------------------------------------------
export function computeDeudaInicial({ visits = [], payments = [], items = [], products = [], periodStartKey = '', dayKeyOf, sellerOf = null }) {
  let ventaHist = 0;
  let cobradoHist = 0;
  for (const v of visits || []) {
    const k = dayKeyOf(v.visit_date || v.created_at);
    if (!k || k >= periodStartKey) continue;
    if (sellerOf && sellerOf(v) !== sellerOf.__filter) continue;
    ventaHist += toNum(v.venta);
    // Cobrado histórico: abono de la visita (efectivo/nequi). Si la visita no
    // trae método, igual cuenta como cobro si hay monto (auditoría conservadora).
  ...
  // Pagos sueltos (tabla payments) anteriores al periodo que no estén ya
  // reflejados como abono en visits: se suman solo si no hay visits ese día
  // con el mismo monto para evitar doble conteo. Aproximación documentada:
  // si el backend ya incluye el pago en v.abono, no se duplica.
  // (Se usa solo para auditoría de orden de magnitud.)
  void payments;
  void items;
  void products;
...
export function computePeriodRows({
  ...
  const rows = (days || []).map((d) => {
    ...
    // BLOQUE 2 primero (caja del día, independiente de la deuda).
    const cash = sumDayCash(dayVisits, dayPayments);
    // BLOQUE 3 datos (venta/costo del día).
    const sale = sumDaySale(dayVisits, (items || []).length > 0
      ? (items || []).filter((it) => new Set(dayVisits.map((v) => v.id)).has(it.visit_id))
      : [], products, dayPayments, cash.total);

    const venta = sale.venta;
    const costo = sale.costo;
    const costoCll = venta; // valor de venta de lo entregado hoy
    const tieneMovimiento = venta > 0 || cash.total > 0;

    // BLOQUE 1: solo días con movimiento toman/actualizan el arrastre.
    const saldoAnt = tieneMovimiento ? arrastre : 0;
    const credit = buildCreditRow({ deudaInicial: saldoAnt, entregadoCreditoHoy: costoCll, cobradoHoy: cash.total });
    ...
    const entregaOld = tieneMovimiento ? saldoAntOld + cobrosOld - cash.total : 0;

    if (tieneMovimiento) {
      arrastre = credit.entrega;
      arrastreOld = entregaOld;
      ventaPeriodo += venta;
      cobradoPeriodo += cash.total;
      costoPeriodo += costo;
    }

    // BLOQUE 2 cierre (gasto opcional default 0 + validación $).
    ...
    return {
      ...
      pctEfect: cashRow.total > 0 ? Math.round((cash.efectivo / cashRow.total) * 100) : 0,
    };
  });
